...
const isSimilar = (str1, str2, threshold = 0.8) => {
    ...
    if (longer.length - shorter.length > longer.length * (1 - threshold))
        return false;
    const distances = Array(shorter.length + 1).fill(null).map(() => Array(longer.length + 1).fill(null));
...
const extractRawData = (text, originalText) => {
...
const findExactMatch = (extractedData, originalText) => {
    ...
    if (nameMatches.length > 0) {
        ...
        if (exactMatch) {
            ...
            const mostSimilar = nameMatches.reduce((prev, curr) => isSimilar(curr, extractedData.name) ? curr : prev, extractedData.name);
    ...
    if (orgMatches.length > 0) {
        ...
        if (exactMatch) {
            ...
            const mostSimilar = orgMatches.reduce((prev, curr) => isSimilar(curr, extractedData.organization) ? curr : prev, extractedData.organization);
    ...
    if (addressMatches.length > 0) {
        ...
        if (exactMatch) {
            ...
            const mostSimilar = addressMatches.reduce((prev, curr) => isSimilar(curr, extractedData.address) ? curr : prev, extractedData.address);
    ...
    if (mobileMatches.length > 0) {
        ...
        if (exactMatch) {
            ...
            const mostSimilar = mobileMatches.reduce((prev, curr) => isSimilar(curr, extractedData.mobile) ? curr : prev, extractedData.mobile);
